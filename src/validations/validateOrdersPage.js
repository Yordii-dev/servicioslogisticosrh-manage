...
import {
  ...
  validateOrdersManagement,
} from './validateParts'

//Controllers       -> Models
//validateHandlePage -> validateParts

const validateOrdersPage = async () => {
  try {
    //Pages to return
    let login = {
      name: 'login',
    ...
    let validateOrdersManagem = await validateOrdersManagement()
    if (validateOrdersManagem.status == 'fail')
      return { validate: validateOrdersManagem, pageToReturn: handle }
